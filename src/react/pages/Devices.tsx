...
const Devices: React.FC = () => {
  ...
  const connectWifi = (serial: string) => {
    void connectionFeedback(deviceManager.connectWifi(serial), "Failed to connect with tcp to device " + serial);
  };

  const connectTcp = (connectIp: string) => {
    localStorage.setItem(LAST_IP_KEY, connectIp);
    void connectionFeedback(deviceManager.connectTcp(connectIp), "Failed to connect to address" + connectIp);
  };

  const pair = (pairIp: string, pairCode: string) => {
    void connectionFeedback(
      deviceManager.pair(pairIp, pairCode),
      "Failed to pair with device " + pairIp + " and code " + pairCode + ". But maybe it is already paired."
    );
  };
  ...
  return <div>
    ...
    <div style={{ padding: "0 20px" }}>
      ...
      <a href="https://developer.android.com/tools/adb?hl=pt-br#connect-to-a-device-over-wi-fi" target="_blank" rel="noreferrer">
    ...
    {result.devices.length === 0 ? (
      ...
      <div style={{ padding: "0 20px" }}>
        <DevicesList devices={result.devices} onConnect={() => void getDevices()} onConnectWifi={connectWifi} />
        {result.deviceInfo && <DeviceInfoCard deviceInfo={result.deviceInfo} />}
        <UsersList users={result.users} />
        <h2>Installed Games</h2>
        <div className="game-list">
          {(result.apps.map(game => gamesManager.getGameFromCache(game.packageName)).filter(game => game) as Game[]).map(game => <GameCard
            game={game}
            key={game.id}
          />)}
        </div>
