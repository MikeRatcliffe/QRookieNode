...
const Library: React.FC = () => {
  ...
  const updateDownloadedGames = async () => {
    ...
    setDownloads(result.map(id => gamesManager.getGameFromCacheById(id)).filter(game => game) as Game[]);
  };
  ...
  return <>
    ...
    {downloading.length === 0 && downloads.length === 0 && <div
      style={{
        display: "flex",
    ...
    <div style={{ display: "flex", flexDirection: "row", flexWrap: "wrap", padding: "1em" }}>
      ...
      {downloads.length !== 0 && <div style={{ flex: 1 }}>
        ...
        <div className="game-list">
          {downloads.map(game => <GameCard downloaded game={game} key={game.id} onSelect={() => void navigate(`/games/${game.id}`)} />)}
        </div>
      </div>}
    </div>
  </>;
